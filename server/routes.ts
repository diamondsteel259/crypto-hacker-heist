...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/user/:userId/equipment/purchase", validateTelegramAuth, verifyUserAccess, async (req: AuthRequest, res) => {
    const { userId } = req.params;
    ...
    try {
      const { isUserAdmin } = await import('./middleware/auth');
      const isAdmin = await isUserAdmin(String(req.telegramUser!.id));
      ...
      const result = await db.transaction(async (tx: any) => {
        ...
        // Admins get free purchases!
        if (!isAdmin && currentBalance < et.basePrice) {
        ...
        if (!isAdmin) {
          ...
          // Admin just gets the hashrate increase without paying
          await tx.update(users)
            .set({
              totalHashrate: sql`${users.totalHashrate} + ${et.baseHashrate}`
            })
        ...
        return { ...equipment, adminPurchase: isAdmin };
      });
