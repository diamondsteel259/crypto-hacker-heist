...
export default function BlockExplorer() {
  ...
  const activeMiners = latestBlock?.totalMiners || 0;

  const formatHashrate = (hr: number) => {
    if (hr >= 1000000) return `${(hr / 1000000).toFixed(2)} PH/s`;
    if (hr >= 1000) return `${(hr / 1000).toFixed(2)} TH/s`;
    return `${hr.toFixed(2)} GH/s`;
  };

  const getUserShareForBlock = (blockNum: number) => {
    const reward = userRewards.find(r => r.block.blockNumber === blockNum);
    return reward ? {
